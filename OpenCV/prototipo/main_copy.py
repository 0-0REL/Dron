...
# pid
dist_pid = PID(0.1,0.001,0.01,30,output_limits=(-1,1))
yaw_pid = PID(0.5,0.01,0.01,0,output_limits=(-360,360))
...
def get_heading_thread(dron):
    ...
    while True:
        ...
        time.sleep(1/10)  # Consulta cada 100 ms

# camara
cam = cv2.VideoCapture(0)

# Detección con OpenCV DNN (Caffe)
net = cv2.dnn.readNetFromCaffe("/home/rodrigo/Documentos/Dron/complementos/modelos/deploy.prototxt", "/home/rodrigo/Documentos/Dron/complementos/modelos/res10_300x300_ssd_iter_140000_fp16.caffemodel")
in_width = 300
in_height = 300
mean = [104, 117, 123]
conf_threshold = 0.7
...
# Inicia el thread para actualizar heading
threading.Thread(target=get_heading_thread, args=(dron,), daemon=True).start()
win_name = "Camara"
cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)

# Abre el archivo CSV para guardar los datos
#log_file = open("log_prog.csv", "w", newline="")
#csv_writer = csv.writer(log_file)
#csv_writer.writerow(["timestamp", "desp"])
...
while True:
    ...
    if not okt:
        ...
        if bbox:
            ...
            z = 1-0.01*yc
            if z < 0.5: z = 0.5
            vx, yaw, vz = dist_pid(a/1000), yaw_pid(xc), z
            v = np.array([vx, 0, vz])
            rot = np.diag([1, 1,-1]) @ rot_z(math.radians(heading))
            vp = rot @ v
            #print(rot)
            print(f'vx: {vp[0]:.2f}, vy: {vp[1]:.2f}, vz: {vp[2]:.2f}, heading: {heading:.2f}',end='\r')
            #print(yaw, xc)
            ...
            #csv_writer.writerow([timestamp, z])
        else:
            dron.mav.set_position_target_local_ned_send(
                0,
                dron.target_system,
