...
El \textbf{controlador PD} tiene la siguiente estructura
\begin{equation}
	\label{eq:c_pd}
...
El \textbf{controlador PID} tine la siguiente estructura
\begin{equation}
	\label{eq:c_pid}
...
\textbf{Seguimiento de referencia en control en espacio de estados}
\\Precompensador discreto \cite{fadaliServo2013}
\begin{equation}
	\mathbf{u}(k) = -\mathbf{Kx}(k)+\mathbf{Nr}(k)
\end{equation}
\begin{equation}
	\mathbf{N = [C_d[I-A_d+B_dK]^{-1}B_d]^{-1}}
\end{equation}
Modelo Interno para entrada escalón \cite{fadaliServo2013}
\begin{equation}
	\label{eq:c_mi}
	\begin{bmatrix}
		\mathbf{x}(k+1) \\
	...
	\mathbf{u}(k)
\end{equation}
\[\mathbf{u}(t) = \mathbf{-Kx}(k)-\mathbf{K_ee}(k)\]
...
\begin{equation}
	\psi = \operatorname{atan2}(mag_y,mag_x) - \delta
\end{equation}
