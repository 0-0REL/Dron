...
class blazeFaceDetector():
    ...
    def drawDetections(self, img, results):
        ...
        for boundingBox, keypoints, score in zip(boundingBoxes, keypoints, scores):
            ...
            x2 = int(self.img_width * boundingBox[2])
            ...
            #	xKeypoint = int(keypoint[0] * self.img_width)
            #	yKeypoint = int(keypoint[1] * self.img_height)
    ...
    def prepareInputForInference(self, image):
        ...
        reshape_img = img_input.reshape(1, self.inputHeight, self.inputWidth, self.channels).astype(np.float32)
    ...
    def filterWithNonMaxSupression(self, boxes, keypoints, scores):
        ...
        selected_indices = cv2.dnn.NMSBoxes(
            boxes.tolist(), scores.tolist(), self.scoreThreshold, self.iouThreshold
     )
        if len(selected_indices) == 0:
            filtered_boxes = np.empty((0, 4))
            filtered_keypoints = np.empty((0, KEY_POINT_SIZE, 2))
            filtered_scores = np.empty((0,))
        else:
